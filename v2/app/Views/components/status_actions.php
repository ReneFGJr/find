<div class="card shadow-sm">
    ...
    <div class="card-body d-grid gap-2">
        ...
        foreach ($statusList as $status):
            $css_status = 'outline-primary';
            if (isset($itemInfo['i_status']) && $itemInfo['i_status'] == $status['id_is']) {
                $css_status = 'primary';
            }
            ?>
            <a href="<?= base_url('catalog/status/' . ($itemInfo['id_i'] ?? '')) . '?status=' . $status['id_is'] ?>"
                class="btn btn-<?= $css_status; ?> w-100 mb-1" onclick="return confirm('Deseja realmente encaminhar para o status <?= htmlspecialchars($status['is_name']) ?>?');">
