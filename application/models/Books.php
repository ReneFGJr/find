...
function vitrine($dt=array())
{
	...
	if (count($dt) > 0)
	{
		...
		for ($r=0;$r < count($dt);$r++)
			{
				if ($r > 0) { $wh .= ' or ';}
				$wh .= '(i_manifestation = '.$dt[$r].') ';
			}
		$sql = "
		select i_manifestation, i_identifier, i_titulo, max(id_i) as idi
			from find_item
			where i_library = '".LIBRARY."' and i_manifestation <> 0
			and ($wh)
			group by i_manifestation, i_identifier, i_titulo
			order by idi desc
			limit 96
			";
	} else {
...
function manual_api($id)
{
	...
	if (strlen($title) > 0)
	{
		...
		$dt = $this->le_item($id);
		redirect(base_url(PATH.'v/'.$dt['i_manifestation']));
	}
}
...
function exemplar($isbn)
{
	$sql = "select max(i_exemplar) as i_exemplar,
					max(i_manifestation) as i_manifestation
			from find_item
			where i_identifier = '$isbn'
			and i_library = '".LIBRARY."'";
	...
	if (count($rlt) > 0)
	{
		...
		if (round($line['i_exemplar']) > 0)
			{
					$ln = $this->le_manifestacao($line['i_manifestation']);
					$rs = array(
					$line['i_exemplar'],
					$line['i_manifestation'],
					$ln['i_titulo']);
				return($rs);
			} else {
				return(array(0,0,''));
