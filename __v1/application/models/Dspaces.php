...
class dspaces extends CI_model {
	...
	//var $path = "U:\\dspace\\collection_raul_pila";
	var $path = "U:\\Arq.02\\output";
	var $handle = '';
	var $thumbnai = '';
	var $files = array();
	...
	function tools($path = '') {
		$sx = '<a href="' . base_url('index.php/dspace/tools_dir_created') . '" class="btn btn-secondary">' . msg('itens_create_directory') . '</a>';
		return ($sx);
	}

	function tools_directory_create($path = '',$sz=4) {
		$sx = 'OK';
		$file = array();
		$path = $this->path;
		$dest = 'u:\\dspace\\collection_raul_pila_1\\';
		$d = dir($path);
		$idd = 1;
		$xdir = '';
		while (($file = $d -> read()) !== false) {
			$zdir = substr($file,0,4);
				...
				if ($xdir != $zdir)
					{
						...
						if ($idd > 0)
							{
								$dir = $this->create_dir($idd,$dest);
								$xdir = $dir;
								$d_file = $dir.'\\'.$file;
								$o_file = $path.'\\'.$file;
								//echo '<br>'.$o_file.' => '.$d_file;
